...
def load_data(name, root, cache_size=10):
...
def validate(epoch, model, valloader, file=False):
    ...
    with torch.no_grad():
        for ix, (x,y) in enumerate(valloader):
            x, y = x.to(args.gpu), y.to(args.gpu).unsqueeze(1)
            yhat = model(x)
            u, v = yhat[:,0],yhat[:,1]
            preds.append(yhat)
            true.append(y) 
    ...
    if args.rank==0:
        ...
        if file:
            torch.save((preds.cpu(),true.cpu()),os.path.join('./results',f"{args._name}_{os.environ['SLURM_JOB_ID']}_Epoch{epoch}.pt"))
        dist.barrier()
    else:
        dist.barrier() 
    ...
    for epoch in range(args.epochs):
        ...
        for (large_x, large_y) in trainloader:
            ...
            for index in range(0, large_x.shape[0]-args.batch_size, args.batch_size):
                x, y = large_x[index:index+args.batch_size], large_y[index:index+args.batch_size].unsqueeze(1)
