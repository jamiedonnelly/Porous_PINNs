...
def validate(epoch, model, valloader, file=False):
    ...
    with torch.no_grad():
        for ix, (x,y) in enumerate(valloader):
            x, p = x.to(args.gpu), y.to(args.gpu).unsqueeze(1)
            phat = model(x)
            preds.append(phat)
            true.append(p) 
    ...
    for epoch in range(args.epochs):
        ...
        for (large_x, large_y) in trainloader:
            ...
            for index in range(0, large_x.shape[0]-args.batch_size, args.batch_size):
                x, p = large_x[index:index+args.batch_size], large_y[index:index+args.batch_size].unsqueeze(1)
                
                optim.zero_grad()
                phat = ddp_model(x)
                loss = criterion(phat,p)
                loss.backward()
                optim.step()
